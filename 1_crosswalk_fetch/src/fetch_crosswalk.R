...
#' @param depth_ind indicator file for Navico depths file
...
fetch_navico_points <- function(out_ind, depth_ind, temp_ind) {
  ...
  navico_depth_data <- scipiper::sc_retrieve(depth_ind) %>%
  ...
  navico_temp_points_sf <- readxl::excel_sheets(navico_temp_file) %>%
    ...
    mutate(SocialMapMaxDepthMeters = NA, SocialMapMeanDepthMeters = NA)
  ...
  navico_depth_points_sf <- st_as_sf(navico_depth_data, coords = c('CenterLong','CenterLat'), crs=4326) %>%
    mutate(site_id = sprintf("Navico_%s", MapWaterbody_ID), .keep="unused", .before=1) %>%
    dplyr::select(site_id, SocialMapMaxDepthMeters, SocialMapMeanDepthMeters)
  ...
  bind_rows(navico_temp_points_sf, navico_depth_points_sf) %>%
    group_by(site_id) %>% summarize(
      ...
      SocialMapMaxDepthMeters = dplyr::last(SocialMapMaxDepthMeters),
      SocialMapMeanDepthMeters = dplyr::last(SocialMapMeanDepthMeters)) %>%
    saveRDS(file = outfile)
