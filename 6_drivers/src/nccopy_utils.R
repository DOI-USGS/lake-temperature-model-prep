...
nccopy_split_combine <- function(nc_filepath, max_steps = 100){
  nc_filename <- basename(nc_filepath)
  nc_combined_unl_filepath <- file.path(tempdir(), "NLDAS_COMBINED_unl.nc")
  ...
  for (split_file in split_file_metadata){
    ...
    output <- system(sprintf("nccopy -w %s %s", nldas_url, nc_temp_file_fixed))
    if(output | file.size(nc_temp_file_fixed) == 0){
      unlink(nc_temp_file_fixed)
      # try one more time
      message('retrying ', nldas_url)
      output <- system(sprintf("nccopy -w %s %s", nldas_url, nc_temp_file_fixed))
    }
  ...
  system(sprintf('ncrcat -h --overwrite %s %s', split_filepaths, nc_combined_unl_filepath))


  if (file.exists(nc_filepath)){
    unlink(nc_filepath) # doesn't seem to honor the overwrite arg and gets hung up on interactive mode
  }
  # then make the time dim fixed again on the larger file
  system(sprintf("ncks --fix_rec_dmn time %s -o %s", nc_combined_unl_filepath, nc_filepath))
...
create_nldas_url <- function(t0, t1, x0, x1, y0, y1, variable){
  ...
  sprintf(paste0(base_url, query_url), x0, x1, t0, t1, y0, y1, var, t0, t1, y0, y1, x0, x1)
}
