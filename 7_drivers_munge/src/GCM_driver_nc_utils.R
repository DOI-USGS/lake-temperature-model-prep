...
generate_gcm_nc <- function(nc_file, gcm_raw_files, vars_info, grid_params, spatial_info,
  ...
  source_info <- list('title' = sprintf("Notaro debiased downscaled GCM data - %s", gcm_name),
                      'source' = paste("These data were derived from a source raster dataset of dynamically downscaled",
                                       "global climate model (GCM) data generated by Michael Notaro (mnotaro@wisc.edu)",
                                       "and others (see https://ccr.nelson.wisc.edu/dynamical-downscaling/index.php).",
                                       "For internal USGS users, the source dataset can be accessed at this url:",
                                       sprintf("http://gdp-netcdfdev.cr.usgs.gov:8080/thredds/ncss/notaro_debias_%s/dataset.html.", tolower(gcm_name)),
