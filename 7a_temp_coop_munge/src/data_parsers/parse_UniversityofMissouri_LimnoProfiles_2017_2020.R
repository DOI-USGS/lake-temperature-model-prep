...
parse_UniversityofMissouri_LimnoProfiles_2017_2020 <- function(inind, outind) {
  ...
  dat_2017 <- files_2017 %>% 
    map_dfr(~ read_subset_consistent_data(., keep_cols = keep_cols_2017)) %>% 
    mutate(
  ...
  dat_2018_2020 <- files_2018_2020 %>% 
    map_dfr(~ read_subset_consistent_data(., keep_cols = keep_cols_2018_2020)) %>% 
    mutate(
  ...
  dat_2018_hw <- files_2018_hw %>% 
    map_dfr(~ read_subset_hw_files(., skip_rows = 0)) 
  
  all_hw_files <- rbind(dat_2017_hw, dat_2017_hw_092, dat_2018_hw) %>% 
    mutate(
      DateTime = date,
      time = time,
      Timezone = NA,
