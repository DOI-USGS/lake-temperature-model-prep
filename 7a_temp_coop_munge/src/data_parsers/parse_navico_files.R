parse_navico_files <- function(inind, outind){
  ...
  # clean data
  clean <- raw %>% 
    filter(!is.na(`Hour (UTC)`)) %>% # remove temporally aggregated data
    mutate(date_complete = 
    ...
    mutate(time = format(date_complete, "%H:%M")) %>% 
    select(DateTime, time, TimeZone, depth, temp = AveWaterTempC, id) 
