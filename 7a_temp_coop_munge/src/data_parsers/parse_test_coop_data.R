parse_test_coop_data <- function(inind, outind) {
  ...
  raw_file <- read.csv(infile, header=T)
    ...
    clean <- raw_file %>%
    mutate(date = as.Date(date, format = "%m/%d/%Y"), temp=fahrenheit_to_celsius(temp_deg_f), depth = convert_ft_to_m(as.numeric(depth_ft)), time=as.character(time), lakeid=as.character(lakeid)) %>%
