...
plot_lake_obs_map <- function(fileout_base, temp_obs_ind = '7b_temp_merge/out/temp_data_with_sources.feather.ind', lakes_sf_fl, us_states_ind, plot_crs = "+init=epsg:2811"){

  # Read the data
  temp_data <- feather::read_feather(scipiper::as_data_file(temp_obs_ind)) # replace as_data_file with sc_retrieve when practical
  conus_states <- readRDS(scipiper::sc_retrieve(us_states_ind)) %>% st_transform(plot_crs)
...
plot_lake_obs_map(
  ...
  us_states_ind = '2_crosswalk_munge/out/us_states_sf.rds.ind',
